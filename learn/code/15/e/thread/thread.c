...
static bool elem2thread_info(struct list_elem* pelem, int ) {
    ...
    return false;	// 此处返回 false 是为了迎合主调函数 list_traversal, 只有回调函数返回 false 时才会继续调用此函数
}
...
void thread_init(void) {
    ...
    // 先创建第一个用户进程:init
    process_execute(init, "init");         // 放在第一个初始化, 这是第一个进程, init 进程的 pid 为 1
