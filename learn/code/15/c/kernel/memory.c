...
// WEI: pde 0 与 768 映射在同一个页表，所以 0 也会有一个映射到相同的物理地址,
// 我觉得不如选择 0xc0400000 769 开始 这样就不会有重合
...
struct arena {
    ...
    uint32_t cnt;
    bool large; // large 为 ture 时, cnt 表示的是页框数。否则 cnt 表示空闲 mem_block 数量
};
...
uint32_t* pte_ptr(uint32_t vaddr) {
    // 先访问到页表自己 + 再用页目录项 pde(页目录内页表的索引) 做为 pte 的索引访问到页表 + 再用 pte 的索引做为页内偏移
    uint32_t* pte = (uint32_t*)(0xffc00000 +((vaddr & 0xffc00000) >> 10) + PTE_IDX(vaddr) * 4);
    return pte;
}

// 得到虚拟地址 vaddr 对应的pde的指针
uint32_t* pde_ptr(uint32_t vaddr) {
    // 0xfffff 是用来访问到页表本身所在的地址
...
static void page_table_add(void* _vaddr, void* _page_phyaddr) {
    ...
    // 页目录项和页表项的第 0 位为 P,此处判断目录项是否存在, 1 存在 0 不存在
    if (*pde & 0x00000001) {
        ...
        *pde = (pde_phyaddr | PG_US_U | PG_RW_W | PG_P_1);
        ...
        /* 分配到的物理页地址 pde_phyaddr 对应的物理内存清0,
         * 避免里面的陈旧数据变成了页表项,从而让页表混乱.
         * 访问到 pde 对应的物理地址,用 pte 取高 20 位便可.
         * 因为 pte 是基于该 pde 对应的物理地址内再寻址,
...
// 从内核物理内存池中申请 pg_cnt 页内存,成功则返回其虚拟地址, 失败则返回NULL
...
// 在用户空间中申请 pg_cnt 个 4k 内存, 并返回其虚拟地址
...
void mfree_page(enum pool_flags pf, void* _vaddr, uint32_t pg_cnt) {
    ...
    // 确保待释放的物理内存在低端 1M + 1k大小的页目录 + 1k大小的页表地址范围外
...
static void mem_pool_init(uint32_t all_mem) {
    ...
    // 页表大小 = 1页的页目录表 + 第 0 和第 768 个页目录项指向同一个页表 +
    // 第 769~1022 个页目录项共指向 254 个页表, 共 256 个页框
    uint32_t page_table_size = PG_SIZE * 256;
    uint32_t used_mem = page_table_size + 0x100000;	  // 0x100000 为低端 1M 内存
    uint32_t free_mem = all_mem - used_mem;
    // 1 页为 4k, 不管总内存是不是 4k 的倍数, 对于以页为单位的内存分配策略，不足 1 页的内存不用考虑了。
    ...
    // 全局或静态的数组需要在编译时知道其长度，而我们需要根据总内存大小算出需要多少字节。所以改为指定一块内存来生成位图.
