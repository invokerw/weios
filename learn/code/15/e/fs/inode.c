...
static void inode_locate(struct partition* part, uint32_t inode_no, struct inode_position* inode_pos) {
    ...
    uint32_t off_size = inode_no * inode_size;	    // 第inode_no 号 I 结点相对于 inode_table_lba 的字节偏移量
    ...
    if (left_in_sec < inode_size ) {	  // 若扇区内剩下的空间不足以容纳一个 inode, 必然是 I 结点跨越了 2 个扇区
...
void inode_sync(struct partition* part, struct inode* inode, void* io_buf) {	 // io_buf 是用于硬盘 io 的缓冲区
    ...
    ide_read(part->my_disk, inode_pos.sec_lba, inode_buf, count);
    memcpy((inode_buf + inode_pos.off_size), &pure_inode, sizeof(struct inode));
    ide_write(part->my_disk, inode_pos.sec_lba, inode_buf, count);
}
...
struct inode* inode_open(struct partition* part, uint32_t inode_no) {
    ...
    // 需要将 inode 置于内核空间, 故需要临时将 cur_pbc->pgdir 置为NULL
    ...
    if (inode_pos.two_sec) {	// 考虑跨扇区的情况
        ...
        // i 结点表是被 partition_format 函数连续写入扇区的, 所以下面可以连续读出来
