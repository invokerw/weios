...
int32_t file_create(struct dir* parent_dir, char* filename, uint8_t flag) {
    ...
    create_dir_entry(filename, inode_no, FT_REGULAR, &new_dir_entry);	// create_dir_entry 只是内存操作不出意外,不会返回失败

    // 同步内存数据到硬盘
    // a 在目录 parent_dir 下安装目录项 new_dir_entry, 写入硬盘后返回true,否则false
...
rollback:
    ...
    if(rollback_step >= 1) {
        // 如果新文件的i结点创建失败,之前位图中分配的 inode_no 也要恢复
...
int32_t file_write(struct file* file, const void* buf, uint32_t count) {
    ...
    if (add_blocks == 0) {
        ...
        if (file_has_used_blocks <= 12 ) {	// 文件数据量将在 12 块之内
