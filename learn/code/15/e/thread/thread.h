...
* 初始情况下此栈在线程自己的内核栈中位置固定，
* 在 PCB 所在页的最顶端，每次进入中断时就不一定了
********************************************/
struct intr_stack {
    ...
    uint32_t esi;
    uint32_t ebp;
    uint32_t esp_dummy;	 // 虽然 pushad 把esp也压入,但 esp 是不断变化的,所以会被 popad 忽略
    uint32_t ebx;
    uint32_t edx;
    uint32_t ecx;
    uint32_t eax;
...
struct task_struct {
    ...
    uint8_t ticks;	   // 每次在处理器上执行的时间 tick 数

    // 此任务自上cpu运行后至今占用了多少 cpu tick, 也就是执行时间
    ...
    int32_t  exit_status;         // 进程结束时自己调用 exit 传入的参数
