...
static void partition_format(struct partition* part) {
    ...
    uint32_t last_size = SECTOR_SIZE - (block_bitmap_last_byte % SECTOR_SIZE);	     // last_size 是位图所在最后一个扇区中，不足一扇区的其余部分
    ...
    // 由于 inode_table 中共 4096 个inode, 位图 inode_bitmap 正好占用1扇区,
    ...
    i->i_sectors[0] = sb.data_start_lba;	     // 由于上面的 memset, i_sectors 数组的其它元素都初始化为 0
...
int32_t sys_lseek(int32_t fd, int32_t offset, uint8_t whence) {
    ...
    switch (whence) {
        case SEEK_SET: // SEEK_SET 新的读写位置是相对于文件开头再增加 offset 个位移量
            new_pos = offset;
            break;

        case SEEK_CUR: // SEEK_CUR 新的读写位置是相对于当前的位置增加 offset 个位移量, 可正负
            new_pos = (int32_t)pf->fd_pos + offset;
            break;

        case SEEK_END:	   // SEEK_END 新的读写位置是相对于文件尺寸再增加 offset 个位移量，应为负值
...
// 读取目录 dir 的 1 个目录项, 成功后返回其目录项地址, 到目录尾时或出错时返回NULL
...
int32_t sys_stat(const char* path, struct stat* buf) {
    ...
    int32_t ret = -1;	// 默认返回值
    struct path_search_record searched_record;
    memset(&searched_record, 0, sizeof(struct path_search_record));   // 记得初始化或清 0, 否则栈中信息不知道是什么
    int inode_no = search_file(path, &searched_record);
    if (inode_no != -1) {
...
void filesys_init() {
    ...
    while (channel_no < channel_cnt) {
        ...
        while(dev_no < 2) {
            ...
            while(part_idx < 12) {   // 最多支持 4 个主分区 + 8 个逻辑
