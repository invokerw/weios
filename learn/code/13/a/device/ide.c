...
static void select_sector(struct disk* hd, uint32_t lba, uint8_t sec_cnt) {
    ...
    // 写入 lba 地址(即扇区号)
    outb(reg_lba_l(channel), lba);		     // lba 地址的低8位,不用单独取出低 8 位. outb 函数中的汇编指令 outb %b0, %w1 会只用 al。
...
static void write2sector(struct disk* hd, void* buf, uint8_t sec_cnt) {
    ...
    // insw 的参数是字，也就是 2 个字节，因此要将扇区数转换成字后再调用 insw。先转换成字节，再将字节除以 2 就是字了
...
static void identify_disk(struct disk* hd) {
    ...
    // 硬盘参数信息是以字为单位的，包括偏移、长度的单位都是字，
    // 在这 16 位的字中，相邻字符的位置是互 换的，所以通过函数做转换
